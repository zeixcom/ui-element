...
`
	...
	private async handleFileChange(filePath: string): Promise<void> {
		...
		try {
			...
			// Check if this is a dependency change that affects other files
			const dependentFiles = this.findDependentFiles(filePath)
			const filesToProcess =
				dependentFiles.length > 0
					? [filePath, ...dependentFiles]
					: [filePath]

			// Get applicable plugins for all files to process
			const allApplicablePlugins = new Set<string>()
			for (const file of filesToProcess) {
				const plugins =
					this.context.buildSystem.getApplicablePlugins(file)
				plugins.forEach(p => allApplicablePlugins.add(p.name))
			}
			const pluginNames = Array.from(allApplicablePlugins)
			...
			if (dependentFiles.length > 0) {
				console.log(
					`🔗 Dependency change detected in ${filePath}, rebuilding ${dependentFiles.length} dependent files...`,
				)
			}

			console.log(
				`🔄 Processing ${filesToProcess.length} file(s) with plugins: ${pluginNames.join(', ')}`,
			)
			...
			// Process all files
			for (const file of filesToProcess) {
				const result = await this.context.buildSystem.buildFile(file)
				results.push(result)
				...
				if (result.success) {
					...
					if (
						...
						result.content !== 'template-updated'
					) {
			...
			if (allSuccessful) {
				...
				this.eventEmitter?.emit('build:error', {
					error: {
						message:
							firstError?.errors?.[0]?.message || 'Build failed',
						code: 'PLUGIN_BUILD_ERROR',
						file: firstError?.filePath || filePath,
					} as any,
					files: filesToProcess,
				})
			}
	 ...
	 */
	private findDependentFiles(changedFilePath: string): string[] {
		const dependentFiles: string[] = []
		const dependencyGraph = this.context.buildSystem.getDependencyGraph()

		// Check if the changed file is a template file
		if (
			changedFilePath.includes('templates') &&
			changedFilePath.endsWith('.ts')
		) {
			// For template changes, we need to find all markdown files that depend on templates
			for (const [filePath, dependencies] of dependencyGraph.entries()) {
				if (
					dependencies.some(
						dep => dep.includes('templates') && dep.endsWith('.ts'),
					)
				) {
					if (
		...
		for (const [filePath, dependencies] of dependencyGraph.entries()) {
			if (
				dependencies.includes(changedFilePath) &&
		...
		return Array.from(new Set(dependentFiles))
	}
