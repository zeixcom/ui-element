...
const DEFAULT_CONFIG: DevServerConfig = {
	...
	watch: {
		...
		paths: [
			{
				directory: 'docs-src/pages',
				extensions: ['.md'],
				label: '📝',
				buildCommands: ['build:docs-html'],
			},
			{
				directory: 'docs-src/components',
...
export class ConfigManager {
	 ...
	 */
	public async load(): Promise<DevServerConfig> {
		if (this._config) {
			return this._config
		}
	 ...
	 */
	public async reload(): Promise<DevServerConfig> {
		this._config = null
		return this.load()
	}
...
/**
 * Utility function to get build commands for a file
 */
export function getBuildCommands(filePath: string): string[] {
