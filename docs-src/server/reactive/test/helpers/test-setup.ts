...
export function mockConsole(): MockConsole {
	...
	console.log = (...args: any[]) => {
		logCalls.push(args)
		// Still output to console in tests for debugging
	...
	return {
		log: {
			calls: logCalls,
			clear: () => (logCalls.length = 0),
		},
		error: {
			calls: errorCalls,
			clear: () => (errorCalls.length = 0),
		},
		warn: {
			calls: warnCalls,
			clear: () => (warnCalls.length = 0),
		},
		info: {
			calls: infoCalls,
			clear: () => (infoCalls.length = 0),
		},
		getLogs: () => {
			return [
				...logCalls.map(call => call.join(' ')),
				...errorCalls.map(call => call.join(' ')),
				...warnCalls.map(call => call.join(' ')),
				...infoCalls.map(call => call.join(' ')),
			]
		},
		restore: () => {
