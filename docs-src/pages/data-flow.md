...
<div>
  <p class="lead"><strong>Learn how El Truco components can work together seamlessly.</strong> Start with simple parent-child relationships, then explore advanced patterns like custom events and shared state. Build modular, loosely coupled components that communicate efficiently.</p>
  {{ toc }}
</div>
</section-hero>
...
Then, we need to convert the total of all product quantities to a string and pass it on to the `BasicButton` component. In El Truco we use the `pass()` function to share state across components:
...
This enables full type checking, autocompletion, and access to El Truco component methods like `.getSignal()` and `.setSignal()`.
  ...
  'shopping-cart',
  {
    items: fromEvent(
      ...
      ({ event, source, value }) => {
        ...
        // Can access El Truco methods like source.getSignal('quantity')
        const newItem = {
          id: event.detail.id,
...
Each El Truco component should declare its own `HTMLElementTagNameMap` extension:
...
- **Access to El Truco methods** like `.getSignal()` and `.setSignal()`
