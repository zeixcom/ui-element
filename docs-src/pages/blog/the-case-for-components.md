...
**El Truco** builds on the Web Platform rather than abstracting over it. We embrace semantic HTML as the foundation. Add CSS to your components however you like and make them look great. Use Web Components to add reusable behavior.
...
This manual wiring works, but notice how much boilerplate we needed for a simple reactive property. As components grow more complex, this pattern becomes harder to maintain. This is where **El Truco** shines – it provides the reactivity patterns you need while staying close to the platform.

Here's the same component rewritten using El Truco. Notice how the concerns are cleanly separated:
...
```js
import { component, asString, on, setText } from '@zeix/el-truco'
...
Components in El Truco are still standard Web Components that can be used everywhere. Other components need to know nothing more about El Truco components other than they are standard HTML elements with a few reactive properties according to a clearly defined contract – its public interface.
...
- **El Truco** builds on this foundation, simplifying common tasks like wiring inputs and syncing state to the DOM.
...
We'll look at the pitfalls of imperative state wiring as logic grows, and how El Truco's signal graph provides a robust and minimal foundation for keeping your user interface in sync.
