import { all, asInteger, component, first, insertOrRemoveElement, on, setText, State, state } from '../../../'

export default component('calc-table', {
	columns: asInteger(),
	rows: asInteger(),
}, el => {
	const colHeads = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
	const rowTemplate = el.querySelector<HTMLTemplateElement>('.calc-table-row')
	const colheadTemplate = el.querySelector<HTMLTemplateElement>('.calc-table-colhead')
	const cellTemplate = el.querySelector<HTMLTemplateElement>('.calc-table-cell')
	...
	for (let i = 0; i < el.columns; i++) {
		columns.set(colHeads[i], state(Array.from({ length: el.rows }).map(() => 0)))
	}

	const getColumnArray = (rowKey: string): number[] => {
		return Array.from(el.querySelectorAll<HTMLInputElement>(`tbody input[data-key="${rowKey}"]`))
			.map(input => Number.isFinite(input.valueAsNumber)? input.valueAsNumber : 0)
	}
	...
	return [
		...
		/* Create rows */
		first('tbody',
			insertOrRemoveElement(target => el.rows - target.querySelectorAll('tr').length, {
				create: parent => {
		...
		first('thead tr',
			insertOrRemoveElement(target => el.columns - (target.querySelectorAll('th').length - 1), {
				create: parent => {
					...
					columns.set(colKey, state(Array.from({ length: el.rows }).map(() => 0)))
		...
		all('tbody tr',
			insertOrRemoveElement(target => el.columns - target.querySelectorAll('td').length, {
		...
		all('tbody input',
			on('change', e => {
				const colKey = (e.target as HTMLInputElement)?.dataset['key']
		...
		all('tfoot td',
			setText((target: HTMLTableCellElement) => String(
				columns.get(target.dataset['key']!)?.get()
					.reduce((a, b) => a + b, 0) ?? 0
			))
		)
    ]
