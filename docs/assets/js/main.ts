...
class MyCounter extends UIElement {
	...
	connectedCallback() {
		this.set('parity', () => this.get('count') ?? 0 % 2 ? 'odd' : 'even')
        this.first('.increment').on('click', () => this.set('count', (v?: number) => v ? ++v : 1))
        this.first('.decrement').on('click', () => this.set('count', (v?: number) => v ? --v : 0))
...
class HelloWorld extends UIElement {
	connectedCallback() {
		...
		this.first('input').on('input', (_target, _index) => (e: Event) => this.set('name', (e.target as HTMLInputElement).value ?? ''))
	}
}
...
class MySlider extends UIElement {
	connectedCallback() {
		...
		this.all('.slide')
			.sync((host, target, idx) => toggleClass(
				'active',
				() => idx === this.get('active')
			)(host, target))
		this.all('.dots span')
...
class TabList extends UIElement {
	...
	connectedCallback() {
		...
		this.all('menu button')
			.sync((ui, idx) => setProperty('ariaPressed', () => this.get('active') === idx)(ui))
...
class AccordionPanel extends UIElement {
	connectedCallback() {
		...
		this.self
		    .sync(toggleAttribute('open'))
			.sync(toggleAttribute('collapsible'))
			.sync(setProperty('ariaHidden', () => !this.get('open') && !this.get('collapsible')))

		// Control inner details panel
		this.first('details')
			.sync(setProperty('open'))
			.sync(setProperty('ariaDisabled', () => !this.get('collapsible')))
	}
}
...
export class InputRadiogroup extends UIElement {
	...
	connectedCallback() {
		...
		this.all('label').sync(ui => toggleClass(
			'selected',
