...
type Parser<T extends {}> = (host: HTMLElement, value: string | null, old?: string | null) => T;
type Initializer<T extends {}> = T | Parser<T> | ((host: HTMLElement) => T | ComputedCallback<T>);
 ...
 */
declare const component: <P extends ComponentProps>(name: string, init: { [K in string & keyof P]: Initializer<P[K]>; } | undefined, setup: ComponentSetup<P>) => Component<P>;
...
export { type ComponentProps, type Component, type Parser, type Initializer, type Provider, RESET, component, first, all, pass, on, emit };
